...
var extraSeries = chart.addAreaSeries({
  ...
  crossHairMarkerVisible: false,
         ...
         async function getData2() {
             const moralisRawData = await Moralis.Cloud.run("SellPriceBTCDGS")
             dataJSON = JSON.stringify(moralisRawData);
             var dataParse = JSON.parse(dataJSON);
             const cdata = dataParse.map(d => {
                 return {
                     time: (d.Timestamp),
                     value: (d.SellPriceBTCDGS)
                 }
             });
             let sortedcdata = cdata.slice().sort((a, b) => a.time - b.time);
         ...
         firstRow.innerHTML = '<div style="font-size: 24px; margin: 4px 0px; color: #fff"> Price (BTC) </div>';
         ...
         chart.subscribeCrosshairMove((param) => {
             if (param.time) {
               ...
               const price2 = param.seriesPrices.get(extraSeries);
                 firstRow.innerHTML = '<div style="font-size: 24px; margin: 4px 0px; color: #fff"> Price (BTC) </div>' + '  ' + '<div style="font-size: 15px; margin: 4px 0px; color: #fff">Buy Price ' + price.toFixed(2) + ' BTC <br>Sell Price ' + price2.toFixed(2) + ' BTC</div></div>';
             } else {
                 firstRow.innerHTML = '<div style="font-size: 24px; margin: 4px 0px; color: #fff"> Price (BTC) </div>' ;
             }
         });    